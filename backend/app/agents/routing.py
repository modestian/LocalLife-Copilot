...
class IntentRouter:
    ...
    def _predict(self, query: str, history_summary: str | None) -> IntentOutput | None:
        ...
        prompt = (
            "将输入分类为 knowledge_query、tool_use 或 general_chat。探店、购物、找店"
            "和评价摘要属于 knowledge_query；只有明确要求调用工具才属于 tool_use。"
            f"\n历史摘要：{clipped}\n用户输入：{query}"
        )
        try:
...
class ClarificationPlanner:
    ...
    def plan(self, state: ChatState) -> ClarificationDecision:
        ...
        if not missing:
            return ClarificationDecision(False)
        labels = {"budget_cent_per_person_lte": "人均预算", "party_size": "人数"}
        examples = []
...
def _rule_based_intent(
    ...
    # Heuristic: lifestyle-intent verb + object → knowledge query
    if re.search(
        r"(?:想吃|想喝|想吃点|想喝点|来点|吃点|喝点|想买|想逛|想做|想剪|想找|想配|想修|想试|想去|找找)\S{0,10}",
        query,
    ):
        return ChatIntent.KNOWLEDGE_QUERY
...
def _extract_distance(text: str) -> int | None:
    ...
    if not matches:
        return 3000 if "附近" in text else None
    values = [
