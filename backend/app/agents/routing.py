...
_KNOWLEDGE_MARKERS = (
    "推荐", "探店", "找店", "找一家", "搜一下", "哪里吃", "吃什么", "餐厅", "饭店",
    "馆子", "菜馆", "咖啡店", "评价", "评论", "点评", "口碑", "差评", "好评", "槽点",
    "亮点", "人均", "预算",
    "公里", "千米", "附近",
)
_REVIEW_MARKERS = ("评价", "评论", "点评", "口碑", "亮点", "槽点", "差评", "好评")
...
_CUISINE_ALIASES: Mapping[str, str] = {
    "川菜": "川菜", "四川菜": "川菜", "火锅": "火锅", "粤菜": "粤菜",
    "广东菜": "粤菜", "湘菜": "湘菜", "湖南菜": "湘菜", "鲁菜": "鲁菜",
    "江浙菜": "江浙菜", "本帮菜": "本帮菜", "日料": "日本料理",
    "日本料理": "日本料理", "寿司": "日本料理", "韩餐": "韩国料理",
    "韩国料理": "韩国料理", "西餐": "西餐", "意大利菜": "意大利菜",
    "泰国菜": "泰国菜", "烧烤": "烧烤", "烤肉": "烤肉", "海鲜": "海鲜",
    "素食": "素食", "咖啡": "咖啡", "甜品": "甜品",
}
_ATMOSPHERE_ALIASES: Mapping[str, str] = {
    "安静": "安静", "清静": "安静", "热闹": "热闹", "浪漫": "浪漫",
    "有氛围": "有氛围", "氛围感": "有氛围", "复古": "复古", "文艺": "文艺",
    "私密": "私密", "高档": "高档", "休闲": "休闲",
}
_SCENE_ALIASES: Mapping[str, str] = {
    "约会": "约会", "聚会": "聚会", "团建": "团建", "商务宴请": "商务宴请",
    "谈事": "商务会谈", "商务会谈": "商务会谈", "亲子": "亲子", "带娃": "亲子",
    "家庭聚餐": "家庭聚餐", "独食": "独食", "一个人吃": "独食",
    "下午茶": "下午茶", "夜宵": "夜宵",
}
_CHINESE_DIGITS = {
...
def _extract_distance(text: str) -> int | None:
    matches = re.findall(
        r"(\d+(?:\.\d+)?)\s*(公里|千米|km|KM|米|m)(?:以内|内|范围内|之内)?", text
    )
    if not matches:
