...
def _source_name(source_uri: str, fallback: str) -> str:
    """Choose the controlled file name used for MIME and loader selection.

    ``source_key`` is normally a content hash, which intentionally has no file
    extension. The persisted URI is generated by the upload endpoint and ends
    with the sanitized original filename, so it is the appropriate source for
    extension-based validation without trusting caller-supplied metadata.
    """
...
class WorkerLifecycleService:
    ...
    def _ingest(self, job: LifecycleJob) -> Mapping[str, JsonValue]:
        ...
        source_name = _source_name(job.source_uri, job.source_key)
