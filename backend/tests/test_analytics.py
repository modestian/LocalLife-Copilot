...
class TestCompareMerchantsService:
    ...
    async def test_compare_with_date_range(self, service: AnalyticsService) -> None:
        ...
        m001 = next(m for m in result["merchants"] if m["merchant_id"] == "M001")
        assert m001["sample_count"] == 1
        m002 = next(m for m in result["merchants"] if m["merchant_id"] == "M002")
        assert m002["sample_count"] == 0
    ...
    async def test_compare_merchant_with_no_data(self, service: AnalyticsService) -> None:
        ...
        m999 = next(m for m in result["merchants"] if m["merchant_id"] == "M999")
        assert m999["sample_count"] == 0
        assert m999["positive_rate"] == 0.0

    @pytest.mark.asyncio
    async def test_compare_has_metadata_fields(self, service: AnalyticsService) -> None:
        result = await service.compare_merchants(["M001", "M002"])
        assert "period_start" in result
        assert "period_end" in result
        assert "metric_definition" in result
        assert "minimum_sample_size" in result
        assert "insufficient_data" in result
        assert result["metric_definition"] == "sentiment_positive_rate"
...
class TestCompareMerchantsEndpoint:
    def test_success(self, sample_records: list[FakeReviewAnalysis]) -> None:
        ...
        assert [m["merchant_id"] for m in data["merchants"]] == ["M001", "M002"]
        assert "period_start" in data
        assert "period_end" in data
        assert "metric_definition" in data
        assert "minimum_sample_size" in data
    ...
    def test_with_date_range(self, sample_records: list[FakeReviewAnalysis]) -> None:
        ...
        assert response.status_code == 200
        body = response.json()
        m001 = next(m for m in body["data"]["merchants"] if m["merchant_id"] == "M001")
        assert m001["sample_count"] == 1
    ...
    def test_three_merchants(self, sample_records: list[FakeReviewAnalysis]) -> None:
        ...
        response = client.post(
            "/api/v1/merchants/compare",
            json={"merchant_ids": ["M001", "M002", "M003"]},
        )
        assert response.status_code == 200
        body = response.json()
        assert len(body["data"]["merchants"]) == 3
        # M003 has no data → all zeros
        m003 = next(m for m in body["data"]["merchants"] if m["merchant_id"] == "M003")
        assert m003["sample_count"] == 0
